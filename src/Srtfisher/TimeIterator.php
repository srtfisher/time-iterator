...
use Carbon\Carbon;
use Srtfisher\TimeIteratorException;
...
class TimeIterator implements \Iterator, \Countable
{
   ...
   *
   * @param integer|Guzzle Time in seconds to go back (also could be a Guzzle Object)
   * @param integer|Guzzle Time in seconds to continue onto (also could be a Guzzle Object)
   ...
   */
    public function __construct($start = null, $end = null, $intervalTime = null, $callback = null)
    {
        ...
        if ($intervalTime) {
            $this->intervalTime = intval($intervalTime);
        }
   ...
   * Set the Start Time
   *
   * @param  integer|Guzzle Interger to subtract from now or a Guzzle Object
   */
    public function setStart($time = 0)
    {
        $this->start = ($time instanceof Carbon) ? $time : Carbon::now()->subSeconds(intval($time));
    }

  /**
   * Set the End Time
   *
   * @param  integer|Guzzle Interger to add from now or a Guzzle Object
   */
    public function setEnd($time = 0)
    {
        $this->end = ($time instanceof Carbon) ? $time : Carbon::now()->addSeconds(intval($time));
    }
   ...
   */
    ...
    {
        $this->intervalTime = intval($interval);
    }
